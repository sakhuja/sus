...
// <h1><h2></h2></h1>1) Get Request Token
$request = new OAuthRequest('GET', "$OAuthEndPoint/get_request_token", array(
	'oauth_nonce'=>mt_rand(),
	'oauth_timestamp'=>time(),
	'oauth_version'=>'1.0',
	'oauth_signature_method'=>'HMAC-SHA1', //'PLAINTEXT'
...
'Username' and 'Password' which means they authorize your app. On successful login the end users will see a code in the page \n";
...
if(strtoupper($argv[1]) == "JSON")
{
	...
	var_dump($jsonClient->GetUserData(new stdclass()));
}
else if(strtoupper($argv[1]) == "SOAP")
{
	...
	//	var_dump($soapClient->GetUserData(new stdclass()));
	$soapClient->GetUserData(new stdclass());
